(function(window, document) {
     ...
     */ function resizeHandler() {
            ...
            for(var i = particles.length - 1; i >= 0; i--)if (particles[i].position.x > elWidth || particles[i].position.y > elHeight) particles.splice(i, 1);
            // Adjust particle density
            var numParticles = Math.round(canvas.width * canvas.height / options.density);
            if (numParticles > particles.length) while(numParticles > particles.length){
                var p = new Particle();
                particles.push(p);
            }
            else if (numParticles < particles.length) particles.splice(numParticles);
            // Re-index particles
     ...
     */ Particle.prototype.draw = function() {
            ...
            for(var i = particles.length - 1; i > this.stackPos; i--){
                ...
                if (dist < options.proximity) {
                    ...
                    if (options.curvedLines) ctx.quadraticCurveTo(Math.max(p2.position.x, p2.position.x), Math.min(p2.position.y, p2.position.y), p2.position.x + p2.parallaxOffsetX, p2.position.y + p2.parallaxOffsetY);
                    else ctx.lineTo(p2.position.x + p2.parallaxOffsetX, p2.position.y + p2.parallaxOffsetY);
                }
            }
            ctx.stroke();
            ctx.closePath();
     ...
     */ Particle.prototype.updatePosition = function() {
            if (options.parallax) {
                if (orientationSupport && !desktop) {
                    ...
                    var ratioX = (winW - 0) / 60;
                    pointerX = (tiltX - -30) * ratioX + 0;
                    // Map tiltY range [-30,30] to range [0,winH]
                    var ratioY = (winH - 0) / 60;
            ...
            switch(options.directionX){
                ...
                default:
                    ...
                    if (this.position.x + this.speed.x + this.parallaxOffsetX > elWidth || this.position.x + this.speed.x + this.parallaxOffsetX < 0) this.speed.x = -this.speed.x;
                    break;
            }
            switch(options.directionY){
                case 'up':
                ...
                default:
                    // If particle has reached edge of canvas, reverse its direction
                    if (this.position.y + this.speed.y + this.parallaxOffsetY > elHeight || this.position.y + this.speed.y + this.parallaxOffsetY < 0) this.speed.y = -this.speed.y;
                    break;
            }
            // Move particle
...
//# sourceMappingURL=index.38e468d5.js.map
